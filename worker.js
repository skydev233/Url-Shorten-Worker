const config = {
  ...
  safe_browsing_api_key: "AIzaSyDUPvv5t_EU07RWLlo2vZK9SVWz6SKO6zg", //Enter Google Safe Browsing API Key to enable url safety check before redirect.
  expiration_ttl: 0, // Short link expiration time in seconds. 86400 = 24 hours. Set to 0 for no expiration.

  // Domain routing
  short_domain: "2x.lat",    // Apex domain that serves short links (e.g. https://2x.lat/xxx)
  ui_hostname: "link.2x.lat", // Subdomain that serves the shortening interface (e.g. https://link.2x.lat)
  
  // CAPTCHA Configuration
  captcha: {
    enabled: true, // Master switch for CAPTCHA service
    api_endpoint: "https://captcha.gurl.eu.org/api", // CAP Worker API endpoint
    require_on_create: false, // Require CAPTCHA when creating short links
  ...
  async function handleRequest(request) {
    ...
    if (request.method === "POST") {
      ...
      if (typeof(stat) == "undefined") {
        return new Response(JSON.stringify({
          status: 200,
          key: "/" + random_key,
          short_url: "https://" + config.short_domain + "/" + random_key
        }), {
          headers: response_header,
        })
      } else {
    ...
    const isUiHost = hostname === config.ui_hostname
    
    // UI subdomain (e.g. link.2x.lat): only the bare subdomain serves the shortening interface.
    if (isUiHost) {
      // Block link.2x.lat/xxx — the UI subdomain must not resolve short links.
      if (path) {
      ...
      const html = await fetch("https://skydev233.github.io/Url-Shorten-Worker/" + config.theme + "/index.html")
    ...
    // Short-link domain (e.g. 2x.lat): bare domain is blocked, only /xxx resolves.
    if (!path) {
      return new Response(html404, {
        headers: { "content-type": "text/html;charset=UTF-8" },
        status: 404
    ...
    <a href="${requestURL.pathname}">Try again</a>
      ...
      if (config.safe_browsing_api_key) {
        if (!(await is_url_safe(location))) {
          let warning_page = await fetch("https://skydev233.github.io/Url-Shorten-Worker/safe-browsing.html")
      ...
      if (config.no_ref == "on") {
        let no_ref = await fetch("https://skydev233.github.io/Url-Shorten-Worker/no-ref.html")
